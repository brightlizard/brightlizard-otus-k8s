package net.brightlizard.shop.delivery.infrastructure.vertx;
...
import net.brightlizard.shop.delivery.core.service.DeliveryService;
...
public class DeliveryReactor extends AbstractVerticle {
    ...
    private Handler<Message<Object>> scheduleDeliveryHandler() {
        return message -> {
            ...
            LOGGER.info("OBJECT FROM ORDER -> {}", order);
            OrderEventModel scheduledOrder = deliveryService.schedule(order);
            LOGGER.info("OBJECT TO ORDER -> {}", order);
